...
static QString remoteFullPathToLocalAbs(const QString& rootDir, const QString& fullPath)
{
    ...
    QString p = fullPath;
    if (p.startsWith('/')) {
        p.remove(0, 1);  // "RemoteRoot/sub/a.md"
    }
...
static bool writeUtf8TextFile(const QString& absPath, const QString& text, QString* err = nullptr)
{
    ...
    if (n != bytes.size()) {
        if (err) {
            *err = QStringLiteral("write size mismatch");
        }
        return false;
    }
    return true;
      ...
      m_structureMgr(new NoteStructureManager(this))
{
    ...
    connect(
        ...
        this,
        &EditorWindow::onTreeViewContextMenuRequested
    );
    ...
    if (m_splitter) {
        ...
        m_splitter->setStyleSheet(
            ...
            "QSplitter::handle:horizontal:hover { background: "
            "#888888;}"
        );
        // 左右拉伸比例 1:3
    ...
    if (m_rightSplitter) {
        ...
        m_rightSplitter->setCollapsible(1,
                                        true);  // preview 允许收缩到 0
    ...
    // ================== previewHost：历史版本内容预览（默认隐藏）==================
    {
        ...
        // 否则用户没法通过 splitter 从 0 再把它拖出来（因为 widget
        // 不参与布局了）
        ui->previewHost->setVisible(true);

        // 关键点2：解除“最小宽度/最小提示”对 splitter 的限制
        // QWidget/QLayout 通常不会把 widget 压到小于
        // minimumSizeHint，除非 minimumSize=0 或 sizePolicy=Ignore/
        // Ignored。:contentReference[oaicite:1]{index=1}
        ui->previewHost->setMinimumSize(0, 0);
        ui->previewHost->setSizePolicy(QSizePolicy::Ignored, QSizePolicy::Expanding);

        // preview 内部控件也做同样处理，避免它自身的 sizePolicy
        // 限制拖到 0
    ...
    // ===================== historyHost：历史列表 + summary  =====================
    {
        ...
        title->setStyleSheet(
            "QLabel { color:#dddddd; font-weight:600; "
            "padding: 4px 2px; }"
        );
        ...
        m_summaryText->setPlaceholderText(
            QStringLiteral("双击左侧笔记以加载历史；点击历史行以预览版本内容。")
        );
        ...
        connect(
            ...
            this,
            &EditorWindow::onHistoryItemClicked
        );
        ...
        connect(
            ...
            this,
            &EditorWindow::onHistoryContextMenuRequested
        );
    }
    ...
    connect(ui->logoutButton, &QPushButton::clicked, this, &EditorWindow::onLogoutClicked);
    connect(ui->updateButton, &QPushButton::clicked, this, &EditorWindow::onUpdateClicked);
    connect(ui->syncButton, &QPushButton::clicked, this, &EditorWindow::onSyncClicked);
    connect(ui->noteUpdateButton, &QPushButton::clicked, this, &EditorWindow::onNoteUpdateClicked);

    connect(m_http, &HttpManager::logoutResult, this, &EditorWindow::onLogoutResult);

    connect(m_http, &HttpManager::deleteNoteResult, this, &EditorWindow::onDeleteNoteResult);
    connect(m_http, &HttpManager::updateNoteResult, this, &EditorWindow::onUpdateNoteResult);
    connect(
        m_http,
        &HttpManager::getNoteByVersioResult,
        this,
        &EditorWindow::onGetNoteByVersionResult
    );
    connect(
        m_http,
        &HttpManager::getHistoryListResult,
        this,
        &EditorWindow::onGetHistoryListResult
    );

    connect(
        m_http,
        &HttpManager::updateNoteStructureResult,
        this,
        &EditorWindow::onUpdateNoteStructureResult
    );
    connect(
        m_http,
        &HttpManager::fetchNoteStructureResult,
        this,
        &EditorWindow::onFetchNoteStructureResult
    );

    connect(m_http, &HttpManager::networkError, this, &EditorWindow::onNetworkError);

    //  Treeview双击逻辑
    connect(ui->treeView, &QTreeView::doubleClicked, this, &EditorWindow::onTreeItemDoubleClicked);
...
void EditorWindow::setToken(const QString& token) { m_token = token; }
...
void EditorWindow::onLogoutClicked()
{
    ...
    auto ret = QMessageBox::question(this, "Confirm logout", "确定要登出账号吗?");
...
void EditorWindow::onUpdateNoteResult(
    bool ok,
    const QString& msg,
    int remoteId,
...
)
{
    ...
    const bool updated =
        m_structureMgr->setRemoteNoteIdByAbsolutePath(m_rootNode.get(), localAbsPath, remoteId);
    if (!updated) {
        return;
    }
...
void EditorWindow::onHistoryItemClicked(QListWidgetItem* item)
{
    ...
    if (m_token.isEmpty() || m_currentRemoteNoteId <= 0) {
        m_summaryText->setPlainText(QStringLiteral(
            "当前笔记没有 remoteId，或 token "
            "为空，无法拉取历史版本。"
        ));
        return;
    }
...
void EditorWindow::onGetHistoryListResult(
    bool ok,
    const QString& msg,
    const QJsonArray& noteHistoryList
)
{
...
void EditorWindow::onFetchNoteStructureResult(
    bool ok,
...
)
{
    ...
    // 把后端返回的结构写入本地 json 文件
    {
        ...
        if (!f.open(QIODevice::WriteOnly | QIODevice::Truncate)) {
            qDebug() << "Failed to open note_structure.json for write:" << filePath
                     << f.errorString();
            return;
        }
    ...
    int maxIdDummy = 0;
    std::unique_ptr<NoteNode> remoteRoot =
        m_structureMgr->fromJson(noteStruct, maxIdDummy, QString());
    if (!remoteRoot) {
        // 解析失败就退化为：至少重建一次树
        initNoteTree(filePath, rootDir);
        return;
...
// 新建树
void EditorWindow::initNoteTree(const QString& jsonFilePath, const QString& rootDirPath)
{
    int nextId = 1;

    // 用 JSON + 目录生成最新的树结构（会扫描文件夹）
    m_rootNode = m_structureMgr->updateStructureFromDirAndJson(jsonFilePath, rootDirPath, nextId);
 ...
 */
void EditorWindow::updateNoteTree(const QString& jsonFilePath, const QString& rootDirPath)
{
    int nextId = 1;
    ...
    if (!m_rootNode) {
        qDebug() << "updateNoteTree: failed to load note structure "
                    "tree from json.";
...
void EditorWindow::onTreeItemDoubleClicked(const QModelIndex& index)
{
    ...
    qDebug() << "Double clicked note:"
             << "id =" << id << ", remoteId =" << remoteId << ", fullPath =" << fullPath;
    ...
    if (m_token.isEmpty() || m_currentRemoteNoteId <= 0) {
        if (m_summaryText) {
            m_summaryText->setPlainText(QStringLiteral(
                "该笔记没有 remoteId（或 token "
                "为空），无法获取历史列表。"
            ));
        }
    } else {
        // 这里假设 HttpManager 已实现该方法，并发出
        // getHistoryListResult 信号
        m_http->getHistoryList(m_token, m_currentRemoteNoteId);
    }
    ...
    //     "Open note",
    //     QString("id = %1\nremoteId = %2\nfullPath = %3\nabsolutePath =
    //     %4") .arg(id) .arg(remoteId) .arg(fullPath) .arg(absolutePath))
    // ;
}
...
void EditorWindow::onTreeViewContextMenuRequested(const QPoint& pos)
{
    ...
    if (type == "folder") {
        ...
        connect(actDelete, &QAction::triggered, this, [this, index]() { this->deleteNote(index); });
    }
...
void EditorWindow::createNoteUnderFolder(const QModelIndex& index)
{
    ...
    if (!f.open(QIODevice::WriteOnly | QIODevice::NewOnly | QIODevice::Text)) {
        QMessageBox::warning(
            this,
            QStringLiteral("创建失败"),
            QStringLiteral("无法创建文件: %1\n错误: %2").arg(filePath, f.errorString())
        );
        return;
    }
    ...
    // 将新笔记传送到服务器
    m_http->updateNote(m_token, -1, 2, -1, "Note init", initial, filePath);
...
void EditorWindow::deleteNote(const QModelIndex& index)
{
    ...
    if (f.exists()) {
        if (!f.remove()) {
            ...
            errBox.setText(
                QStringLiteral("无法删除文件: %1\n错误: %2").arg(absolutePath, f.errorString())
            );
...
bool EditorWindow::promptChangeSummary(
    const QString& title,
    const QString& hint,
    QString& outSummary
)
...
void EditorWindow::onHistoryContextMenuRequested(const QPoint& pos)
{
    ...
    QString summary;
    if (!promptChangeSummary(
            QStringLiteral("回滚到版本 v%1").arg(version),
...
void EditorWindow::startNextMissingPull()
{
    ...
    if (m_missingPullQueue.isEmpty()) {
        // 拉取完毕：用“json + 本地目录扫描”重建树（让 absolutePath
        // 正确）
