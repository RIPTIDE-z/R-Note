...
class NoteStructureManager : public QObject {
   ...
   public:
    ...
    updateStructureFromDirAndJson(const QString& jsonFile, const QString& rootDir, int& nextIdOut);
    ...
    std::unique_ptr<NoteNode>
    fromJson(const QJsonObject& obj, int& maxIdInOut, const QString& parentPath = QString());

    // 写入remotenoteId
    bool setRemoteNoteIdByAbsolutePath(NoteNode* node, const QString& absPath, const int remoteId);
